...
// Fungsi untuk menghasilkan query yang lebih relevan
function buildSearchQuery(prompt) {
  ...
  // Jika ada kata "slot", "roulette", "poker", "blackjack", "dice", "card" – langsung pakai
  ...
  for (const word of filtered) {
    if (casinoKeywords.includes(word)) {
      return word; // pakai kata spesifik
    }
  }
  // Fallback: gabungkan 2 kata pertama
  if (filtered.length >= 2) return `${filtered[0]} ${filtered[1]}`;
  if (filtered.length === 1) return filtered[0];
  return 'casino'; // default
}

// Gambar default base64 (slot machine kecil) – jika semua gagal
const DEFAULT_IMAGE_BASE64 = 'data:image/jpeg;base64,/9j/4AAQSkZJRgABAQAAAQABAAD/2wBDAAg...'; // placeholder

async function generateImage(prompt) {
  // 1. COBA DALL-E (jika tersedia)
  try {
    const response = await client.images.generate({
      model: 'dall-e-3',
      prompt: `A high-quality, professional featured image for an article about: "${prompt}". The image should be related to casino, gambling, slots, or card games.`,
      size: '1024x1024',
      quality: 'standard',
      n: 1,
    });
  ...
  // 3. FALLBACK – GAMBAR SLOT MACHINE DARI PIXABAY (selalu online)
  ...
  // 4. ULTIMATE FALLBACK – BASE64 minimal (chip casino)
